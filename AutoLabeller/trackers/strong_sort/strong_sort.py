...
class StrongSORT(object):
    def __init__(self, 
        ...
        self.tracker = Tracker(
            metric, max_iou_distance=max_iou_distance, max_age=max_age, n_init=n_init, max_unmatched_preds=max_unmatched_preds, ema_alpha=ema_alpha, mc_lambda=mc_lambda)




    def update_base(self, xywhs, confs, clss, ori_img):
    ...
    def update_with_cocos(self, cocos, ori_img):
        xywhs=np.array([[x["bbox"][0]+x["bbox"][2]/2, x["bbox"][1]+x["bbox"][3]/2, x["bbox"][2], x["bbox"][3]] for x in cocos])
        confs=np.array([coco["score"] for coco in cocos])
        classes=np.array([coco["category_id"] for coco in cocos])
        clss=torch.from_numpy(classes)
        return self.update_base(xywhs, confs, clss, ori_img)
