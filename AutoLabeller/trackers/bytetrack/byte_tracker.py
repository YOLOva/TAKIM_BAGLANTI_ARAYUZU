...
class BYTETracker(object):
    ...
    def update(self, dets, _):
        ...
        self.update_base(confs, xywh, classes,_)

    
    def update_with_cocos(self, cocos, ori_img):
        ratioy=1088/1920
        ratiox=612/1080
        bboxes=np.array([[x["bbox"][0]+x["bbox"][2]/2, x["bbox"][1]+x["bbox"][3]/2, x["bbox"][2], x["bbox"][3]] for x in cocos])
        #bboxes=np.array([x['bbox'] for x in cocos])
        xywh=torch.from_numpy(bboxes)
        confs=np.array([coco["score"] for coco in cocos])
        classes=np.array([coco["category_id"] for coco in cocos])
        return self.update_base(confs,xywh, classes, ori_img)


    def update_base(self, confs, xywh, classes, _):
        self.frame_id += 1
        activated_starcks = []
        refind_stracks = []
        lost_stracks = []
