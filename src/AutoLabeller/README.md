### Diğer Autolabeller'i de test edin. Bu auto labellerin iyi çalışabilmesi için görsellerin zamana bağlı ard arda gelmesi fps sayısının mümkün olduğunca fazla olması gerekir. Fps düştüğünde nesneler tespit edilemeyebilir.
https://github.com/YOLOva/AutoLabellerSAHI


# Bu yazılım Altlarda bulunan başlıklarda bulunan github reposunda bulunan cisim takip sistemi temel alınarak yazılmıştır. Temel amaç Daha önce eğitilen bir nesne tespit modeli ile etiketleme dosyaları oluşturmaktır. Nesne takip yazılımının nesne tespitini iyileştirdiği düşünülmektedir.
...
- Bundan sonraki kısımlarda bu sanal ortam kısmının kesinlikle aktif olması gerekmektedir!!! Active değilse lütfen aktifleştirin!.
```
conda create --name myenv 
conda activate myenv
```
## requirements gereksinimler yüklenir.
```
cd AutoLabeller
...
# Çalıştırma Video üzerinde
Video üzerinde çalıştırmak için daha önce oluşturduğunuz ortam aktifken aşğıdaki komut çalıştırılır.

```
python auto_video_labeller.py --source [video yolu] --output-folder [videonun label ve görsel dosyalarının yazılacağı klasör]
```


# Çalıştırma Framelerine ayrılmış klasör üzerinde
Hali hazırda framelerine ayrılmış klasör üzerinde çalıştırmak için daha önce oluşturduğunuz ortam aktifken aşğıdaki komut çalıştırılır.

```
python auto_labeller.py --source [framelerin bulunduğu klasör dizini] --output-folder [labellerin yazılacağı klasör]
```


# Bundan sonra bulunan başlıklar Nesne izleme yazılımına aittir.
